...
public class CustomTieredMachineBuilder extends SimpleMachineBuilder {
    ...
    private static CustomTieredMachineBuilder[] customTiered(String name,
        ...
        for (int tier : tiers) {
            CustomTieredMachineBuilder register = new CustomTieredMachineBuilder(
                    GTValues.VN[tier].toLowerCase(Locale.ROOT) + "_" + name,
                    holder -> machineConstructor.apply(holder, tier)).tier(tier);
    ...
    public static MachineBuilder<MachineDefinition> createAll(String name, Object... args) {
        ...
        if (args.length > 1) {
            ...
            if (args[0] instanceof BiFunction<?, ?, ?> machineFunction) {
                builders = customTiered(name, (BiFunction<IMachineBlockEntity, Integer, MetaMachine>) machineFunction,
                        tiers);
            } else if (args[0] instanceof BaseFunction machineFunction) {
                // builders = customTiered(name, (BiFunction<IMachineBlockEntity, Integer, MetaMachine>)
